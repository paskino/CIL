...
class TestOperator(CCPiTestClass):
    ...
    def _FiniteDifference(self, backend):
        ...
        res = FD.domain_geometry().allocate(FillType["RANDOM"], backend=backend)
        ...
        res = Id.domain_geometry().allocate(FillType["RANDOM"], backend=backend)
        ...
        u = G.range_geometry().allocate(FillType["RANDOM"], backend=backend)
        res = G.domain_geometry().allocate(None, backend=backend)
        G.adjoint(u, out=res)
        w = G.adjoint(u)
        ...
        u = G.domain_geometry().allocate(FillType["RANDOM"], backend=backend)
        res = G.range_geometry().allocate(None, backend=backend)
        G.direct(u, out=res)
        w = G.direct(u)
